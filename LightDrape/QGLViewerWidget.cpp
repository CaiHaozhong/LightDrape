...
{  
  ...
  set_scene_pos(Vec3d(0.0, 10, 0.0), 1.0);
  onEndInitializeGL();
}
...
{
  onBeginReshapeWindow(_w, _h);
  update_projection_matrix();
  glViewport(0, 0, _w, _h);
  updateGL();
  onEndReshapeWindow(_w, _h);
}
...
void
QGLViewerWidget::set_scene_pos( const OpenMesh::Vec3d& _cog, float _radius )
{
  center_ = _cog;
  radius_ = _radius;
