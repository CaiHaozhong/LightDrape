...
#ifndef __MYIMPORTERT_HH__
#define __MYIMPORTERT_HH__
 ...
 */
template <class Mesh>
class MyImporterT : public BaseImporter
{
public:
  ...
  MyImporterT(Mesh& _mesh) : mesh_(_mesh), halfedgeNormals_() {}
  ...
  virtual FaceHandle add_face(const VHandles& _indices)
  {
    ...
    if (_indices.size() > 2)
    {
      ...
      for (it=_indices.begin(); it!=end; ++it)
        ...
        {
          omerr() << "MyImporterT: Face contains invalid vertex index\n";
          return fh;
        }
      ...
      for (it=_indices.begin(); it!=end; ++it)
        for (it2=it+1; it2!=end; ++it2)
          if (*it == *it2)
          {
            omerr() << "MyImporterT: Face has equal vertices\n";
  ...
  void finish()
  {
    if (!failed_faces_.empty())
    {
      omerr() << failed_faces_.size()
	    << " faces failed, do not add them as isolated faces\n";
